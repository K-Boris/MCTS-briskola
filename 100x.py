...
for ijk in range(0,100):
	...
	while len(deck)>1:
		...
		for h in range(0,2):
			if currentPlayer==0:
				...
				table.append(playerHand[currentPlayer].pop(random.randint(0,2)))
			if h==0:
	...
	for i in range(0,3):
		...
		for h in range(0,2):
			if currentPlayer==0:
				...
				table.append(playerHand[currentPlayer].pop(random.randint(0,len(playerHand[currentPlayer])-1)))
			if h==0:
				currentPlayer = nextPlayer(currentPlayer)
		# evaluate who wins
		currentPlayer = whoWinsReal(table[0],table[1],briskula,currentPlayer)
	...
	print(ijk,': A: ',points[0],' B: ',points[1])
	array[0].append(points[0])
	array[1].append(points[1])
